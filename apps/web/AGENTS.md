...
- **GALLERY_COLLECTION(갤러리 모아보기)**: 운영자가 선택한 여러 공개 게시판을 `전체 + 게시판별` KRDS Line 탭으로 렌더한다. 게시글은 발행일 최신순이며 카드는 게시판 GALLERY 스킨 카드 패턴을 재사용한다. 게시판별 탭 라벨은 `boardTabLabels[boardId]` 우선, 없거나 빈 값이면 게시판명으로 폴백한다.
