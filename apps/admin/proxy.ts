 ...
 *
 * 단일 책임: 모든 요청에 `x-pathname` 헤더를 주입한다.
 * Server Component layout(`/(authenticated)/layout.tsx`)이 `headers().get('x-pathname')`
 * 으로 현재 경로를 읽어, DEMO_MODE에서 `ensureDemoSession`이 splash redirect URL의
