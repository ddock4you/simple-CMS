 ...
 * 시연 스냅샷 설정 페이지 (PR7).
 *
 * - 운영(__PROD__) 또는 dev 환경의 14개 snapshot 대상 모델 row count + Media 합계 사이즈를 미리보기
...
export default async function DemoSnapshotPage() {
  ...
  const stats = await demo.runWithBypass(async () => {
    ...
    const [
      ...
      prisma.user.count({ where: { sessionId: sourceSessionId } }),
      prisma.media.count({ where: { sessionId: sourceSessionId } }),
      prisma.siteSettings.count({ where: { sessionId: sourceSessionId } }),
      prisma.navigationMenu.count({ where: { sessionId: sourceSessionId } }),
      prisma.board.count({ where: { sessionId: sourceSessionId } }),
      prisma.homeSection.count({ where: { sessionId: sourceSessionId } }),
      prisma.subpage.count({ where: { sessionId: sourceSessionId } }),
      prisma.post.count({ where: { sessionId: sourceSessionId } }),
      prisma.pageBlock.count({ where: { sessionId: sourceSessionId } }),
      prisma.homePopup.count({ where: { sessionId: sourceSessionId } }),
    ...
    return {
      rowCounts: {
        ...
        NavigationMenu: navigationMenuCount,
        Board: boardCount,
        HomeSection: homeSectionCount,
        Subpage: subpageCount,
        Post: postCount,
        PageBlock: pageBlockCount,
        HomePopup: homePopupCount,
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mb-4">
          ...
          내보낸 JSON은 14개 snapshot 대상 모델과 sharp로 1600px 리사이즈된 Media
          base64를 포함하며, User 비밀번호와 운영자 식별자(uploadedById)는 자동으로
          제거/익명화됩니다. AuditLog와 ErrorLog는 snapshot에서 제외합니다. 즉시 적용은
