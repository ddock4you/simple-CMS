...
import { useBulkReactivateUsers } from '../api/useUserMutations';
...
export function BulkReactivateUserDialog({ ids, open, onOpenChange, onCompleted }: Props) {
    ...
    <AlertDialog open={open} onOpenChange={handleOpenChange}>
      <AlertDialogContent className="max-w-xl">
        {phase.kind === 'confirm' ? (
          <>
            <AlertDialogHeader>
              <AlertDialogTitle>사용자 일괄 활성화</AlertDialogTitle>
              <AlertDialogDescription>
                선택한 {ids.length}명의 사용자를 활성화하시겠습니까? 정지 상태가 아닌
                사용자는 자동으로 제외됩니다.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel disabled={bulkReactivate.isPending}>취소</AlertDialogCancel>
              <AlertDialogAction
                onClick={handleConfirm}
                disabled={bulkReactivate.isPending || ids.length === 0}
              >
                {bulkReactivate.isPending ? '처리 중...' : `${ids.length}명 활성화`}
              </AlertDialogAction>
            </AlertDialogFooter>
          </>
        ) : (
          <>
            <AlertDialogHeader>
              <AlertDialogTitle>일괄 활성화 결과</AlertDialogTitle>
              <AlertDialogDescription>
                {phase.result.updated.length}명 활성화 완료,{' '}
