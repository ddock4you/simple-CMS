...
import { useBulkUpdateSubpageStatus } from '../api/useSubpageMutations';
...
export function BulkStatusSubpageDialog({
  ids,
  open,
  onOpenChange,
  onCompleted,
  ...
  const bulkStatus = useBulkUpdateSubpageStatus({
    onSuccess: () => {
      onOpenChange(false);
      onCompleted?.();
    },
    ...
    >
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            선택한 {ids.length}개 서브 페이지의 상태를 변경합니다. 이미 같은 상태인
            항목은 무시됩니다.
          </AlertDialogDescription>
        </AlertDialogHeader>
