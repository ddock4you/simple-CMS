...
| 51  | SSR 성능 — RTT 누적 진단 · React `cache(fn)` 요청 스코프 dedup · Server Component 트리 직렬/병렬 fetch · Next.js `dynamic` literal-only + layout 전염성 · `IN`/`hasSome` 통합 · Prisma + pgbouncer connection 협상 비용 | [사전학습](2-develop/18-SSR성능-RTT진단-ReactCache-Server병렬화-동적정적전염-사전학습.md) | — | Stage 18 |
