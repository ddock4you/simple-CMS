...
| 특별편 | 시연 모드 세션 캐시 격리와 basePath 페이지네이션 — QueryClient session keying · stale id 404 · `runWithUserDemoSession` 명시 scope · shadcn anchor basePath 이탈 · 활동이력 sessionId 명시 필터 | [사전학습](2-develop/특별편-시연모드-세션캐시격리-basePath-사전학습.md) | [학습정리](2-develop/특별편-시연모드-세션캐시격리-basePath-학습정리.md) | 시연 운영 디버깅 |
...
| 특별편 | 공개 웹 SEO 모듈화 — Metadata builder · JSON-LD 렌더 컴포넌트 · URL/sitemap/robots 단일 출처 · 보존 리팩터링 테스트 | [사전학습](2-develop/특별편-공개웹-SEO모듈화-Metadata-JSONLD-단일출처-사전학습.md) | [학습정리](2-develop/특별편-공개웹-SEO모듈화-Metadata-JSONLD-단일출처-학습정리.md) | SEO 리팩터링 |
