...
2. **프로젝트 컨벤션 참조**: Root AGENTS.md의 감사 로그 섹션, admin AGENTS.md의 감사 로그 스펙, `apps/admin/app/api/subpage-feedback/export/route.ts`(production-검증 export 패턴) 참조
...
| 권한 | API에서 `requirePermission(resource, 'read')` + UI에서 `usePermission(resource, 'read')`로 버튼 게이팅 (AGENTS.md "API + UI 양쪽" 패턴) |
