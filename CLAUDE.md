...
- **`apps/web/next.config.ts` `serverExternalPackages` ESM/CJS interop 가드** — Next.js turbopack은 server-side 의존성을 자체 번들링하려 시도하는데, jsdom 같이 dynamic require가 많은 native-leaning 패키지는 ESM/CJS interop가 깨져 Vercel serverless에서 `ERR_REQUIRE_ESM`(`@exodus/bytes` encoding-lite.js → html-encoding-sniffer → jsdom) throw → 메인 페이지 500. **`serverExternalPackages: ['isomorphic-dompurify', 'jsdom']` 등록**으로 번들링 건너뛰고 Node.js 표준 모듈 해석 fallback. `isomorphic-dompurify`(`apps/web/src/shared/lib/renderContent.ts`에서 SSR HTML sanitize)가 jsdom을 lazy load하므로 양쪽 모두 등록. **로컬 build 통과해도 production runtime에서 처음 발견되는 케이스라 반드시 Vercel preview/production 검증 필수**
