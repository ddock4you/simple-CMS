...
- **헤더 로고/네비게이션 마크업**: KRDS `Header.Branding`은 `children`을 `.logo` `<h2>` **밖**에 렌더하므로 로고 이미지를 클릭 가능 영역 안에 두려면 그대로 사용 불가. **Stage 7d `RightSidebar`/`ContentSideNavigation` 동일 패턴**으로 KRDS DOM 클래스(`.header-branding > h2.logo > a`)를 차용한 커스텀 JSX(`apps/web/src/widgets/layout/ui/HeaderBranding.tsx`)로 대체. 공개 웹 런타임 헤더는 `apps/web/src/widgets/layout/ui/HeaderChrome.tsx`가 KRDS DOM 클래스와 의미 구조를 직접 렌더한다. `HeaderBranding`은 `로고 + PC 메뉴 slot + 통합검색 + 모바일 전체메뉴` 한 행을 구성하고, 통합검색/전체메뉴의 full-screen layer는 헤더 CSS 스코프 영향을 피하기 위해 `document.body` portal로 렌더한다. 통합검색 트리거는 `/search` fallback href를 유지하되 클릭 시 검색 모달을 연다. 검색 제출 후에만 `/search?q=...` 결과 페이지로 이동한다. 헤더 일반 스타일은 `globals.css`가 아니라 Tailwind utility로 작성하고, KRDS 동작 클래스(`header-branding`, `logo`, `btn-navi sch navi-row`, `btn-navi all navi-row`, `krds-main-menu`, `gnb-*`)만 JSX에 유지한다. 헤더/모바일 전체메뉴 유틸리티에는 `KRDS 소개`(`https://www.krds.go.kr/`) 링크를 코드 상수로 노출한다. KRDS 메이저 업데이트 시 `HeaderChrome`/`HeaderBranding`/`MobileMenuIsland` + 좌우 네비게이션 컴포넌트를 함께 점검
