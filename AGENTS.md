...
| 7e   | 공개 웹 Tailwind utility 도입 + Hero utility 마이그레이션 + 캐러셀 width 회귀 방어 [[상세]](docs/stages/stage-7e.md)                                           | Tailwind utility + KRDS breakpoint(`tablet:`/`desktop:`) 사용 + Hero/Recommended 모든 viewport 정상. 이후 KRDS Tailwind plugin은 제거되고 정규화 KRDS CSS + arbitrary value 정책으로 전환                                                                                                                                                                                                                             | **완료**                 |
