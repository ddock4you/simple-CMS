...
- **헤더 로고/네비게이션 마크업**: KRDS `Header.Branding`은 `children`을 `.logo` `<h2>` **밖**에 렌더하므로 로고 이미지를 클릭 가능 영역 안에 두려면 그대로 사용 불가. **Stage 7d `RightSidebar`/`ContentSideNavigation` 동일 패턴**으로 KRDS DOM 클래스(`.header-branding > h2.logo > a`)를 차용한 커스텀 JSX(`apps/web/src/widgets/layout/ui/HeaderBranding.tsx`)로 대체. `PageLayout`은 `desktopMenuPortalId` / `mobileMenuTriggerPortalId`를 지정하고 `HeaderBranding` 내부 desktop slot + `Header.Navi`로 `로고 + PC 메뉴 + 통합검색 + 모바일 전체메뉴` 한 행을 구성한다. `Header.Navi`가 없으면 모바일 전체메뉴 버튼이 body로 빠져 아이콘 CSS 스코프가 깨지므로 유지 필수. 헤더 일반 스타일은 `globals.css`가 아니라 Tailwind utility로 작성하고, KRDS 동작 클래스(`header-branding`, `logo`, `btn-navi sch navi-row`)만 JSX에 유지한다. 헤더/모바일 전체메뉴 유틸리티에는 `KRDS 소개`(`https://www.krds.go.kr/`) 링크를 코드 상수로 노출한다. KRDS 메이저 업데이트 시 이 컴포넌트 + 좌우 네비게이션 컴포넌트를 함께 점검
