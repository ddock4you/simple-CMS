...
- 홈 `NOTICE`: 선택 게시판 안에서 중요 게시글 1건과 일반 게시글을 `isImportant DESC`, `publishedAt DESC` 기준으로 노출
...
- **헤더 로고/네비게이션 마크업**: KRDS `Header.Branding`은 `children`을 `.logo` `<h2>` **밖**에 렌더하므로 로고 이미지를 클릭 가능 영역 안에 두려면 그대로 사용 불가. **Stage 7d `RightSidebar`/`ContentSideNavigation` 동일 패턴**으로 KRDS DOM 클래스(`.header-branding > h2.logo > a`)를 차용한 커스텀 JSX(`apps/web/src/widgets/layout/ui/HeaderBranding.tsx`)로 대체. 공개 웹 런타임 헤더는 `apps/web/src/widgets/layout/ui/HeaderChrome.tsx`가 KRDS DOM 클래스와 의미 구조를 직접 렌더한다. `HeaderBranding`은 `로고 + PC 메뉴 slot + 통합검색 + 모바일 전체메뉴` 한 행을 구성하고, 통합검색/전체메뉴의 full-screen layer는 헤더 CSS 스코프 영향을 피하기 위해 `document.body` portal로 렌더한다. 통합검색 트리거는 `/search` fallback href를 유지하되 클릭 시 검색 모달을 연다. 검색 제출 후에만 `/search?q=...` 결과 페이지로 이동한다. 데스크톱 GNB는 KRDS Header 기준으로 하위 메뉴가 있는 1depth를 click/Enter 토글로 열고 `.gnb-backdrop.active`로 헤더를 제외한 영역을 dim 처리한다. HEADER 슬롯의 하위 메뉴 보유 1depth는 관리자 메뉴 타입 `GROUP`을 기본으로 사용하고, 실제 이동 링크는 2depth 이하에 둔다. 헤더 일반 스타일은 `globals.css`가 아니라 Tailwind utility로 작성하고, KRDS 동작 클래스(`header-branding`, `logo`, `btn-navi sch navi-row`, `btn-navi all navi-row`, `krds-main-menu`, `gnb-*`, `gnb-backdrop`)만 JSX에 유지한다. 헤더/모바일 전체메뉴 유틸리티에는 `KRDS 소개`(`https://www.krds.go.kr/`) 링크를 코드 상수로 노출한다. KRDS 메이저 업데이트 시 `HeaderChrome`/`HeaderBranding`/`MobileMenuIsland` + 좌우 네비게이션 컴포넌트를 함께 점검
...
- **MediaPicker**: 동일 컴포넌트가 `/media`, 홈 섹션 ImageUrlInput, Tiptap 본문 툴바 3곳에서 재사용
...
- `packages/db/src/demo/snapshotWalker.ts` — `walkSnapshotForRemap(payload, idMap, kind)` in-place mediaId/boardId 재매핑. 위치별 field name 분기(HERO `slides[].mediaId` / BRIEF_INTRO `mediaId` / FREQUENT_MENU `items[].iconMediaId` / NOTICE·GALLERY_COLLECTION `boardId(s)` / IMAGE `imageMediaId` + `items[].imageMediaId` / RICH_TEXT Tiptap recursion / SubpageVersion.snapshot meta+blocks / HomePopup CONTENT). Tiptap image 노드 `attrs.mediaId` 재귀
...
- `packages/db/src/demo/resetSeedData.ts` — `cleanupExpiredSessions`와 분리된 별도 헬퍼. `__SEED__` row 14모델 deleteMany 자식→부모 순 + 과거 snapshot 정책으로 남은 AuditLog/ErrorLog 정리 + cleanupStorage 콜백 위임. RESERVED 가드 우회를 명시적 진입점으로 격리
...
- `packages/db/src/demo/snapshotWalker.test.ts` — 단위 테스트 (HERO/BRIEF_INTRO/FREQUENT_MENU/NOTICE/GALLERY_COLLECTION/IMAGE/RICH_TEXT/HomePopup/SubpageVersion/Post/edge case)
...
| 6   | Step 9 + 11 + CLI       | snapshot export/import 코어 + walker(mediaId/boardId 위치별 분기 — HERO `slides[].mediaId` / BRIEF_INTRO `mediaId` / FREQUENT_MENU `items[].iconMediaId` / NOTICE·GALLERY_COLLECTION boardId(s) / IMAGE `imageMediaId` / RICH_TEXT Tiptap recursion / SubpageVersion.snapshot) + sharp 1600px 리사이즈 + `__SEED__` 단일 출처 무결성(`cleanupSeedFolder()` + `resetSeedData()`) + cuid 재생성 + `uploadedById = null` anonymization + Phase 1(트랜잭션 밖 upload)/Phase 2($transaction) 분리 + `/api/demo/snapshot/{export,import}` + CLI `pnpm demo:export` / `pnpm demo:import` + `demo-snapshot` 권한 리소스 신규 | **완료**                               |
...
| 7e   | 공개 웹 Tailwind utility 도입 + Hero utility 마이그레이션 + 캐러셀 width 회귀 방어 [[상세]](docs/stages/stage-7e.md)                                           | Tailwind utility + KRDS 표준형 breakpoint(`small:`/`medium:`/`large:`/`xlarge:`) 사용 + Hero viewport 정상. 이후 KRDS Tailwind plugin은 제거되고 정규화 KRDS CSS + arbitrary value 정책으로 전환                                                                                                                                                                                                                   | **완료**                 |
...
| 7i   | Swiper 22M 회귀 자동 감지 + LinkTargetInput 승격·적용 + 커스텀 래퍼 showcase 5개 [[상세]](docs/stages/stage-7i.md)                                             | Carousel container resize play로 ResizeObserver 경로 강제 트리거 + slide width assert / LinkTargetInput을 entities/link-target로 승격하여 home-management 남은 링크 입력 필드에 적용 / Admin/Shared 4개 + Admin/Entities/LinkTarget 1개 showcase                                                                                                                                                                  | **완료**                 |
...
| 15b    | `globals.css`에 shadow 토큰 추가 (`--shadow-card/popover`, light/dark 페어). `@theme inline` + `:root` + `.dark`. 기존 shadcn 토큰 무변경. PageToolbar/DialogToolbar는 현재 shadow 없이 `border-b` 하단 경계 사용                                                                                                  | Storybook smoke 56 tests 회귀 0건 확인                                        | **완료** |
| 15c-1  | shadow 토큰 실 컴포넌트 적용: BlockContentView `shadow-card`, TiptapEditor 팝업 3곳 `shadow-popover`. PageToolbar는 이후 정책 변경으로 `shadow-toolbar` 제거 + `border-b` 기준으로 전환 [[상세]](docs/stages/stage-15c.md)                                                                              | `pnpm --filter @simple-cms/admin build` 통과                                  | **완료** |
...
| 단계 | 내용                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | 확인 가능한 것                                                                                   | 상태     |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | -------- |
| 17   | admin 6파일(Colors·Typography·SpacingAndRadius·Breakpoints·Foundations·AdminCustoms) + web 6파일(KrdsColors·KrdsTypography·KrdsSpacing·Breakpoints·Foundations·WebCustomTokens) 디자인 시스템 카탈로그 스토리. web은 이후 KRDS CSS 정규화(`krds-normalized.css`)로 `html { font-size: 62.5% }`를 앱/Storybook 공통 해결하고, KRDS Tailwind plugin override 없이 token catalog를 inline/arbitrary value 기준으로 문서화. admin: shadcn 토큰·shadow·wrapper 정책·레이아웃 구조 시각화. web: KRDS 색상·타이포·간격·브레이크포인트 토큰 카탈로그 | Storybook `Admin/Design System/*` + `Web/Design System/*` 진입 → 각 토큰·레이아웃 표준 시각 확인 | **완료** |
...
| 19   | 메인 페이지 부가 캐러셀 섹션을 추가했던 단계. 현재는 메인 페이지 정리 정책에 따라 해당 섹션 타입과 UI를 제거하고 HERO/BRIEF_INTRO/FREQUENT_MENU/GALLERY_COLLECTION/NOTICE만 유지                                                                                                                                                                                                                                                                                                                                                                                                            | 삭제 대상 섹션은 admin/web/Storybook/seed/demo seed/Prisma enum에서 제거됨                                                                                 | **완료** |
...
| CSS (web)                | KRDS CSS + Tailwind CSS                            | web                 | `krds-uiux` token/common/component CSS를 빌드 전 `krds-normalized.css`로 조합·정규화(root 100% + KRDS rem 0.625배 + `@layer krds-base`) 후 전역 import. KRDS 이미지 자산은 `/assets/krds/img/...`로 public 서빙. KRDS Tailwind plugin은 미사용, Tailwind 기본 토큰 유지. CMS rich text는 `TiptapContent` wrapper에서 Tailwind descendant utility로 렌더링 경계 형성 |
