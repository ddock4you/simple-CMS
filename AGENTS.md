...
| CSS (web)                | KRDS CSS + Tailwind CSS                            | web                 | `krds-react/dist/index.css`를 빌드 전 `krds-normalized.css`로 변환(root 100% + KRDS rem 0.625배) 후 전역 import. KRDS Tailwind plugin은 미사용, Tailwind 기본 토큰 유지. CMS rich text는 `TiptapContent` wrapper에서 Tailwind descendant utility로 렌더링 경계 형성 |
