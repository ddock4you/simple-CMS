...
**캐시 정책**: admin → web 별 인스턴스라 즉시 invalidate 불가. UI/CLAUDE.md에 "**최대 1분 후 반영**" 명시. favicon은 브라우저 캐시로 추가 수일 지연 가능 — `?v=mediaId` cache busting.
...
- [ ] CLAUDE.md 3곳 업데이트 (root + admin + web)
